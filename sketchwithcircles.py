...
if __name__ == "__main__":
    # input - image, numberofcircles,
    if len(sys.argv)<4:
        print "usage sketchwithcircles image outputdir numberofcircles "
    orig=cv2.imread(sys.argv[1])
    nbCircles=int(sys.argv[3])
    ...
    for f in [20,15,10,5,1]:
        ...
        for tries in range(2):
            x=params[:]
            x,b=searchForParameters(outdir+"output_%d_%d"%(tries,f),orig,params,bounds,25)
            once=True
            baseparams=x[:8]
            ...
            for i in range(len(circles)):
                if sum(includeCircle) <= nbCircles:
                    break
                r=np.argmin(deltas)
        ...
        pfd=open(outdir+"foundparams_%d.txt"%(f,),'w')
        print >>pfd,params
        print >>pfd,bounds
        pfd.close()
        params=localopt(optimizeme,params,bounds,[True,]*len(bounds),1)
        z=anArtist.doit(params)
        pfd=open(outdir+"foundparamsfinal_%d.txt"%(f,),'w')
        print >>pfd,params
        print >>pfd,bounds
        pfd.close()
        s=score(z,orig)
        ...
        fd=open("draw_final_%d.py"%(f,),'w')
        print >>fd,prg,
        fd.close()
        fd=open("cdraw_final_%d.py"%(f,),'w')
        print >>fd,zprg,
        fd.close()
        cv2.imwrite("Final.png",z)
