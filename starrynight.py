...
def compressProgram(prg):
   ...
   for d in dataAsList:
       try:
          dataAsPacked+=struct.pack("HHBBB",d[0]+50+(d[2]+30)*512,d[1]+50+(d[3]+30)*512,d[4],d[5],d[6])
       except:
          print "bad datum",d
   ...
   fd=open('testprg.py','wb')
   fd.write(bytearray(b'\xEF\xBB\xBF'))
   for i in range(10,len(dataAsEscaped)+1):
       if dataAsEscaped[i-1] != 92 and dataAsEscaped[i-3:i] != '\\x8' and dataAsEscaped[i-2:i] != '\\x':
          z="a='byte %d is %d,"%(i,dataAsEscaped[i-1])+dataAsEscaped[:i]+"'\n"
          fd.write(z)
   fd.close()
...
x,b=searchForParameters("outputv8a",orig,params0,bounds,10)

pfd=open("foundparamsv8a.txt",'w')
print >>pfd,x
print >>pfd,b
pfd.close()
...
for i in range(len(circles)/2):
    ...
    if len(zprg) <= sizelimit:
        ...
        if len(zprg) <= 1024:
            fd=open("draw_finalv8.py",'w')
            print >>fd,prg,
            fd.close()
            fd=open("cdraw_finalv8.py",'w')
            print >>fd,zprg,
            fd.close()
            break
