...
def compressProgram(prg):
   ...
   for l in prg.split('\n'):
       if re.match("import",l):
         ...
         cprg+=" x=a/m-h\n"
         cprg+=" y=b/m-h\n"
       elif re.match("range\\(31\\)",l):
           l=re.sub("31",l,"h")
           cprg+=l+'\n'
       elif re.match("p+x\\*k",l):
         l=re.sub('p',l,"a%m-h")
         l=re.sub('q',l,"b%m-h")# prg+="  cv2.circle(z,(p+x*k/30,q+y*k/30),r,n.clip((%.1f*c%+.1f*d%+d,%.1f*c%+.1f*d%+d,%.1f*c%+.1f*d%+d),0,255),-1)\n"%(self.rmtx[0,0],self.rmtx[0,1],self.roff[0],self.rmtx[1,0],self.rmtx[1,1],self.roff[1],self.rmtx[2,0],self.rmtx[2,1],self.roff[2])
         cprg+=l+'\n'
       else:
           cprg+=l+'\n'
   return cprg
...
x,b=searchForParameters("outputv9",orig,params0,bounds,40)

pfd=open("foundparamsv9.txt",'w')
print >>pfd,x
print >>pfd,b
pfd.close()
...
for i in range(len(circles)/2):
    ...
    if len(zprg) <= sizelimit:
        ...
        if len(zprg) <= 1024:
            fd=open("draw_finalv9.py",'w')
            print >>fd,prg,
            fd.close()
            fd=open("cdraw_finalv9.py",'w')
            print >>fd,zprg,
            fd.close()
            break
