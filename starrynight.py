...
class artist:
    ...
    def __init__(self,img):
        ...
        C=np.cov(img.reshape(img.shape[0]*img.shape[1],3).T)
...
def compressProgram(prg):
   ...
   for d in dataAsList:
       try:
          dataAsPacked+=struct.pack("HHBBB",d[0]+62+(d[2]+62)*512,d[1]+62+(d[3]+62)*512,d[4],d[5],d[6])
       except:
          print "bad datum",d
          print "becomes",d[0]+62+(d[2]+62)*512,d[1]+62+(d[3]+62)*512,d[4],d[5],d[6]
...
def searchForParameters(outdir,orig,params0,bounds,iterations,filtersize):    
...
#            z=z/scale
            ...
            bounds.append((idx0[1]-60,idx0[1]+60))
            include.append(True)
            params0.append(idx0[0])
            bounds.append((idx0[0]-60,idx0[0]+60))
            include.append(True)
            params0.append(0)
            bounds.append((-60,+60))
            include.append(True)
...
if __name__ == "__main__":
    ...
    params=[orig.shape[0]//2,int(mval[0]),int(mval[1]),int(mval[2]),int(mval[0]),int(mval[1]),int(mval[2]), 5]
    ...
    pfd=open("foundparamsv26_first.txt",'w')
    print >>pfd,params
    print >>pfd,bounds
    pfd.close()
    nbprimitives=40
    ...
    for sizelimit in [1100,1024]:
        for f in [20,15,10,5,1]:
            x=params[:]
            x,b=searchForParameters("outputv26_%d_%d"%(f,sizelimit),orig,params,bounds,nbprimitives,f)
            baseparams=x[:8]
            ...
            pfd=open("foundparamsv26_%d_%d.txt"%(f,sizelimit),'w')
            print >>pfd,params
            print >>pfd,bounds
            pfd.close()
            params=localopt(optimizeme,params,bounds,[True,]*len(bounds),1)
            pfd=open("foundparamsv26final_%d_%d.txt"%(f,sizelimit),'w')
            print >>pfd,params
            print >>pfd,bounds
            pfd.close()
            s=score(z,orig)
            ...
            if len(zprg) <= 1024:
                fd=open("draw_finalv26_%d_%d.py"%(f,sizelimit),'w')
                print >>fd,prg,
                fd.close()
                fd=open("cdraw_finalv26_%d_%d.py"%(f,sizelimit),'w')
                print >>fd,zprg,
                fd.close()
